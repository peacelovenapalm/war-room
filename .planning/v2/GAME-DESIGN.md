...
Status: SYNTHESIZED 2026-07-08 from 8 section drafts + adversarial critiques.
This document has final authority over all 8 section files under
`.planning/v2/sections/*.md`. Where a section file disagrees with this
document, **this document wins** — the section files remain as reference
detail/rationale but are not independently authoritative anymore. See
"Conflict resolutions" (§9) for the record of every override and why.
...
boss manager sim + colony sim (employees have needs/moods) + command-center
RTS-lite + idle empire, all reading the same state.
...
- **ECONOMY:** dual currency. Cash (liquid, earned from real
  ...
  tiers, never spent).
- **EMPLOYEES:** full colony-sim layer. Persistent named characters per
  (machine, project) identity. Traits derived from real behavior stats.
  Levels, moods, needs, work history. Verbs: hire, assign, train, promote,
  break, fire, retire, rehire.
- **BUILDING:** full build mode. Buy floor space, place typed rooms, place
  buffed furniture, adjacency bonuses, walk paths matter.
- **STAKES:** soft-fail. Neglect → mood/morale drops → employees can quit
...
| **Connect catch-up** (server, on WS connect) | a webview connects                                         | one-jump offline-progress calc: decay (mood/needs/grime/reputation) over `min(elapsed, 72h)`, capped world-event catch-up (≤1 event per skipped calendar day), broadcast snapshot |
| **Live tick** (server, coarse interval)      | ≥1 socket connected, every 5 min                           | visible mood/grime/needs decay during a long session; world-event roll; stops the instant the last socket disconnects                                                             |
| **Session** (webview, 20-60min)              | app open                                                   | build mode, dispatch, employee roster, drawer/inspection                                                                                                                          |
| **Check-in** (webview, phone, <2min)         | app opened briefly                                         | `/api/economy/summary` digest: net Cash/Rep delta + top-5 flavor events + morale/grime warnings                                                                                   |
...
- **Check-in (<2 min, phone):** open → offline-progress digest lands as a
  ...
  morale/grime warnings) → one or two taps (claim a contract, send someone
...
**Decay (soft-fail):** `-1 Rep` per full real-calendar day with **zero**
completed turns and **zero** dispatch runs, floored at 0. No decay on any
day with any real activity, however small.
...
perk ladder (5300) ≈ 3 weeks of consistent play. That is deliberate
...
interface Employee {
  ...
  mood: number; // 0-100, derived every read: clamp(0,100, 0.5*energy + 0.3*social + 0.2*hygiene + moodBoost)
  moodBoost: number; // transient event term, clamped [-20,+20], halves each local day —
  // exists because mood is DERIVED; direct mood events (§4.5) write here,
  // never to a derived value
  needs: EmployeeNeeds;
...
### 4.5 Mood, needs, quit (soft-fail)

```ts
DECAY_PER_HOUR = { energyActive: 1, energyIdle: 0.25, social: 0.5, hygiene: 0.2 };
```

Direct mood events write to the transient `moodBoost` term (§4.2), never
to `mood` itself — mood is derived on read, a derived value cannot be
written (an earlier draft had events adding to `mood` directly, which
contradicted its own "derived every read" definition): crisis resolved by
this employee +5, shift LEAN (studio-wide) +5, shift HEAVY −5, promotion
+15, training +3. `moodBoost` halves each local day and clamps to ±20.
...
**Break** is Cash-free, cosmetic + needs-restoring only. **It never blocks
...
| Break                 | 0                     | active                                  | **No — cosmetic/needs only**        |
...
| Kitchen     | 3×2       | 250  | passive company-wide morale-decay-rate ×0.85                      | **global**, ambient   |
...
- `globalBuffs(layout)` — Server Room (+10% Cash company-wide, requires
  furniture inside) and Kitchen (morale-decay ×0.85), evaluated once per
...
`server/src/worldEventStore.ts`, 12-entry weighted table (full table in
`sections/05-world-events-missions.md` §4.1, glyphs corrected per §6.1
above). Selection: weighted-random among events whose per-id min-gap has
elapsed, evaluated on the same coarse live-tick as §2 — most ticks fire
nothing by construction (rare, ambient, not spammy).
...
the mood/needs fields Employees already owns.
...
confirm`); after that it runs unattended on schedule (Autopilot perk
removes this gate for orders created after purchase).
...
every prompt render — the authoritative first-party signal. **A separate,
explicitly-approved edit** (not silently bundled into a repo commit) adds
a ~10-line snapshot write to `~/.pixel-agents/rate-limit-snapshot.json`.
`bin/needs-input-poller.mjs` forwards it to `POST /api/budget/report`.
...
| Autopilot           | 2500 | new orders skip first-fire-confirm        | orders that already required confirm still do |
...
`dispatchTemplateStore.ts`). This resolves a draft's single-blob
`economyEngine.ts` proposal in favor of the pattern every other section
(and the existing codebase) already independently converged on (§9.13).
...
11. **Command/Automation architecture (04 vs 08 G3).** 04's separate
    `chainStore.ts`/`chainOrchestrator.ts` + `budgetStore.ts` 4-perk
    economy is canonical (§7) — richer, and it's the literal mechanism
...
- **Test/gate bar**: server 329/329, webview 158/158, bin 63/63,
  tsc/lint/build clean — never fewer tests after a milestone than before
  it, at HEAD `50f9ef2`.
