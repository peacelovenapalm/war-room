...
# -n on every non-piping ssh call: ssh otherwise drains this script's stdin,
# eating the piped 'ship' confirmation before read sees it (bit us 2026-07-08).
ssh -n "${SSH_OPTS[@]}" "${MINI}" 'echo ok' >/dev/null 2>&1 || fail "cannot reach ${MINI} over SSH (Tailscale up on both ends?)"
...
ssh -n "${SSH_OPTS[@]}" "${MINI}" 'mkdir -p ~/code/war-room/.planning'
...
if ssh -n "${SSH_OPTS[@]}" "${MINI}" 'grep -qs WAR_ROOM_TOKEN= ~/.war-room/env'; then
...
# ── 3. Remote sanity ─────────────────────────────────────────────────────────
ssh -n "${SSH_OPTS[@]}" "${MINI}" 'zsh -lc "command -v node >/dev/null"' \
