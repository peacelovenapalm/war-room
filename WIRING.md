...
of intent. Line numbers re-verified 2026-07-12 @ 16d7eee. Companion docs:
`docs/ARCHITECTURE.md` (the system in layers) and `docs/DATA-SOURCES.md`
(per-source freshness contracts).
...
| ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Claude Code hooks (`~/.claude/settings.json` hook config)                                                                      | `POST /api/hooks/:providerId` (`HOOK_API_PREFIX`, re-exported at `server/src/constants.ts:42`) | Bearer (the hook forwarder sends the token) + optional `X-Machine`/`X-Pid` | `server/src/httpServer.ts:575` (`registerHookRoute`)                          | `server/src/hookEventHandler.ts`                                          |
| needs-input poller (`bin/needs-input-poller.mjs`) — `claude agents --json` every ~15s                                          | `POST /api/agents/poll`                                                                        | Bearer + `X-Machine`                                                       | `server/src/httpServer.ts:651` (`registerPollRoute`), route at line 657       | `server/src/pollStateHandler.ts`                                          |
| Rate-limit snapshot (same poller tick, `bin/rate-limit-snapshot-hook.mjs` writes the file it reads)                            | `POST /api/budget/report`                                                                      | Bearer + `X-Machine`                                                       | `server/src/httpServer.ts:1743` (`registerBudgetRoutes`)                      | `server/src/budgetStore.ts`                                               |
| Transcript tailer (`bin/transcript-tailer.mjs`) — S3 remote live-tail                                                          | `POST /api/tailer/poll`                                                                        | Bearer + `X-Machine`                                                       | `server/src/httpServer.ts:870` (`registerTailerPollRoute`)                    | drains `remoteTailDemand.ts`'s per-machine `TailInstruction` queue        |
| Tailed output chunks (same tailer, once tail-on)                                                                               | `POST /api/agents/output`                                                                      | Bearer + `X-Machine`                                                       | `server/src/httpServer.ts:780`                                                | `server/src/transcriptParser.ts` (token accounting rides this same plane) |
| Dispatch runner (`bin/dispatch-runner.mjs`) — one opt-in instance per machine, own local allowlist `~/.war-room/dispatch.json` | `POST /api/dispatch/poll`                                                                      | Bearer + `X-Machine`                                                       | `server/src/httpServer.ts:973` (`registerDispatchRoutes`), route at line 1007 | `server/src/dispatchStore.ts`                                             |
...
| District STATE.md files (war-room, TWE proof slice) — **deliberately unwired: both nexus candidate sources are stale/absent, see runbook comment at `.planning/runbooks/nexus-war-room-deploy.sh:57-67`** | `WAR_ROOM_DISTRICT_WARROOM_STATE`, `WAR_ROOM_DISTRICT_TWE_STATE` | not mounted — panel renders honest `source:'unknown'`                                                                                                                            | `server/src/districtsProvider.ts`                                                          | `GET /api/districts`                                                             | unauthenticated                       |
