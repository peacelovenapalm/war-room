 ...
 *
 * Reads two operator-authored files from disk and exposes them as one JSON
 * payload for the HUD BRIEFING panel. Both sources live outside this repo on
 * the deploy host, so they are wired via env vars -- never hardcoded paths:
 *
 ...
 *
 * Missing env var, missing file, or a parse failure never throws: that half
